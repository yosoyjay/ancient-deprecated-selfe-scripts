#!/usr/bin/perl
# $Revision: by Grant Law to correctly write boundary classifications
#
# $Revision: by Joseph Zhang for SMS v10.1
#
...
if ( @ARGV < 2 ) {
    print
"Usage: $0 [SMS grid file in meter with boundary information] [.gr3 file in meters] [river1] ... [riverN]\n";
...
print "Finished reading in element info...\n";
$nodeflag = 'yes';
$node     = '';
...
print "Finished reading in node info...\n";
$boundaryflag        = 'yes';
@bound               = ();
@boundlen            = ();
@boundname           = ();
...
print "Finished reading in boundary node lists...\n";

while ( $lines[ ++$i ] !~ /BD/ && $i < @lines ) { print $lines[ $i - 1 ] }
