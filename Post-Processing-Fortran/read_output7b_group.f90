...
!****************************************************************************************
!											*
!	read_output7b with (x,y) read in from station.bp
!
!jl.   file name and # of files to read read in from extract.in
!											*
!       ifort -Bstatic -assume byterecl -O3 -o read_output7b_group read_output7b_group.f90
!****************************************************************************************
!
      program read_out
      parameter(nbyte=4)
...
!      read(*,*) ndays
!     Read model output file and number of days to extract from extract.in
      open(10,file='extract.in',status='old')
      read(10,'(a30)') file63
      read(10,*) ndays
      close(10)
      ...
      read(10,*) 
      read(10,*) nxy
      allocate(x00(nxy),y00(nxy),stat=istat)
...
!...
      ...
      print*, 'i23d=',i23d,' nrec= ',nrec,' ;ivs=',ivs 
...
!        enddo !i
        ...
        if(i23d.eq.2) then
          ...
          enddo !i
          irec=irec+np*ivs
          write(18,'(e16.8,5000(1x,f14.5))')time,(out2(i,1,1:ivs),i=1,nxy)
        else !i23d=3 
          do i=1,nxy
            do j=1,3 !nodes
              nd=node3(i,j)
...
!               Find bottom index
              ...
              do k=kbpl,nvrt
                do m=1,ivs
                  ...
                  enddo !j
                enddo !m
                write(18,*)time/86400,i,k,ztmp(k),(out2(i,k,m),m=1,ivs) 
              enddo !k
            endif !dry/wet
          enddo !i=1,nxy
