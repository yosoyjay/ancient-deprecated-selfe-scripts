...
for i in range(ndays):
  ...
  if (curr_time==start_time):
    ...
    #hack
    rvr['river_flow'] = rvr['river_flow'][0:96,:]
  else:
    tmp = sp.read_cs_data(in_dir+"/river_flow.dat")
    #hack
    tmp['river_flow'] = tmp['river_flow'][0:96,:]
...
meccs_file_util.write_csv(out_dir+"river_flow_"+str(yr)+"_"+str(day)+"_"+str(ndays)+".csv", rvr['river_flow'])
...
if (rvr['river_flow'].ndim > 1):
  ...
  for i in range(rvr['river_flow'].shape[1]):
    mean_flux[:,i] = mc.get_time_avg_data(rvr['river_flow'][:,i], float(dt), float(interval))
else:
  mean_flux = mc.get_time_avg_data(rvr['river_flow'], float(dt), float(interval))
...
#                                      csdata['cs_surf_hvel_dir'][i,:], rvr['river_flow'][0,:]/csdata['cs_areas'][0,:], dt, interval))
  ...
  cs_HnR.append(mc.get_Hansen_Rattray(csdata['cs_surf_salt'][i,:], csdata['cs_bott_salt'][i,:], csdata['davg_salt'][i,:], \
                                      csdata['cs_surf_hvel_dir'][i,:], rflow/csdata['cs_areas'][0,:], dt, interval))
