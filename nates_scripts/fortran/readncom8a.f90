...
! ifort -g -O3 -Bstatic -o readncom8a_intel readncom8a.f90 -Vaxlib -I/usr/local/netcdf/include -L/usr/local/netcdf/lib -lnetcdf
!
! Compilation on canopus01:
! ifort -g -O3 -Bstatic -o readncom8a_canopus readncom8a.f90 -Vaxlib -I/usr/local/include -L/usr/local/lib -lnetcdf
...
!   Output: hotstart.in (itur=3 for ELCIRC; unformatted for MPI SELFE) when ndays=0;
...
!          write(15, *) (i-1)*iylen+j,lon(i,j),lat(i,j),i,j !temp(i,j,2) !-h(i,j) 
!        enddo
!      enddo
