...
!        Nate changed to read form combine_output.in    
! Output: hotstart.in (unformatted binary). This format is different
!         between Intel and AMD!
!
!  ifort -Bstatic -O3 -assume byterecl -o combine_hotstart2 combine_hotstart2.f90

! Revisions: ntracers added; local_to_global_* format changed (v2.0d up).
!===============================================================================
...
!-------------------------------------------------------------------------------
  ...
  allocatable dfq1(:,:),dfq2(:,:),trel0(:,:,:),trel(:,:,:)
!-------------------------------------------------------------------------------
...
!-------------------------------------------------------------------------------
  ...
  open(10,file='combine_hotstart.in',status='old')
  read(10,*)nproc, ntracers
  read(10,'(a)')it_char
...
!    enddo !i 
  ...
  allocate(idry_e(ne_global),we(nvrt,ne_global),tsel(2,nvrt,ne_global), &
           ...
           dfq1(np_global,nvrt),dfq2(np_global,nvrt),stat=istat)
...
!-------------------------------------------------------------------------------
  ...
  do irank=0,nproc-1
    ...
    ihot_len=nbyte*(4+((6+4*ntracers)*nvrt+1)*ner(irank)+(8*nvrt+1)*nsr(irank)+(3+20*nvrt)*npr(irank))
    ...
    read(36,rec=1)time,it,ifile,(idry_e(ielg(irank,i)),(we(j,ielg(irank,i)),tsel(1:2,j,ielg(irank,i)), &
     ...
     &dfq2(iplg(irank,i),j),j=1,nvrt),i=1,npr(irank)) 
    close(36)
  enddo !irank
...
! Output
  ...
  do i=1,np_global
    write(36) i,eta2(i),idry(i),(tnd(j,i),snd(j,i),tem0(j,i),sal0(j,i),q2(i,j),xl(i,j), &
             dfv(i,j),dfh(i,j),dfq1(i,j),dfq2(i,j),j=1,nvrt)
  enddo !i
  close(36)
