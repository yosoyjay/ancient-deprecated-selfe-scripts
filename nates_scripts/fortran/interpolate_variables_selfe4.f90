...
!	       vgrid.fg: vgrid for fg.bp (SELFE SZ);
!              interpolate_variables_selfe.in: ifile (1: elev; 2: S,T; 3: uv),ndays;				
!       Output: *3D.th format (binary): ifile=1, fort.16 (elev); ifile=2, fort.17 and fort.18 (T,S); 
!               ifile=3, fort.17 (uv).
!               fort.11: fatal errors; fort.12: non-fatal errors.
!               Use timeint_3Dth_2.f90 to change time step in these outputs.
!									
...
!     Open outputs
      if(ifile==1) then
        nrecl=nbyte*(1+npfg)
        open(16,file='fort.16',access='direct',recl=nrecl,status='replace')
      else if(ifile==2) then
        nrecl=nbyte*(1+npfg*nvrt_fg)
        open(17,file='fort.17',access='direct',recl=nrecl,status='replace')
        open(18,file='fort.18',access='direct',recl=nrecl,status='replace')
      else !uv
