        ...
        for j in range(DIV*n_days):
            ...
            if _input == '':
                exit("Error: Unable to read data from %s/%s line %i" 
                     "Check to see if data is available for run day" % (DATA_PATH,file,line_num))
            _input = _input.split()
...
def genFluxTemp(startDate, nDays, grid, dt, exist):
# Get day numbers in "Julian" days and run_times
    s_day     = datetime.date(startDate[0],startDate[1],startDate[2])
    endDate   = s_day + datetime.timedelta(days=(nDays-1))
    endDate   = endDate.timetuple()

    start_jd  = startDate[7]
    end_jd    = endDate[7] 
    n_days    = end_jd - start_jd + 1
    run_times = range(dt, (n_days*86400)+1, dt)

    if startDate[0] > 2010 or exist == 1:
        print("Temp.th and flux.th must be manually created for year %s" % startDate[0])
        print("I'm assuming that it exists in ../../")
        os.system("cp ../../*.th ./")    
        return
...
# Grid 15 (hi-res) & 27 (26c) ends at Beavery Army... so just treat it the same as 22
    if(grid==15 or grid==27):
        grid = 22
...
# grab start and number of days 
    if len(sys.argv) != 6:
        sys.exit("usage: %s [begin_date mm-dd-yyyy] [number of days in run] "
                 "[grid (15, 22, 26, or 27)] [time_step] [*.th exist]" % sys.argv[0])
    else:
        start = sys.argv[1]
        nDays = int(sys.argv[2])
        grid  = int(sys.argv[3])
        dt    = int(sys.argv[4])
        exist = int(sys.argv[5])
...
# Python style dates for script.
    ...
    genFluxTemp(startDate, nDays, grid, dt, exist)
