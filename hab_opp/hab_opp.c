...
{
    FILE *fhab_all, *felv, *fvel, *fsal, *ftmp, *fhab_elv, *fhab_vel0, *fhab_vel1, *fhab_vel2, *fhab_sal0, *fhab_sal1, *fhab_sal2, *freg, *freg_hab, *fhab_tmp0, 
*fhab_tmp1, *fhab_tmp2, *fnod_hab;
    char fname[200], grtitle[200], inputDIR[200], outputDIR[200], filter[200];
    double MIN_ELV, MAX_ELV, MIN_SAL, MAX_SAL, MIN_VEL, MAX_VEL, MIN_TMP, MAX_TMP;
    double u, v, max_sal, min_sal, max_vel, min_vel, max_tmp, min_tmp, mag, ttime, lyr_dpth, *el_areas;
    double avg_sal, avg_vel, avg_tmp;
    double *hab_elv, *hab_sal0, *hab_sal1, *hab_sal2, *hab_vel0, *hab_vel1, *hab_vel2, *hab_tmp0, *hab_tmp1, *hab_tmp2,*hab_all;
    ...
    if (argc != 6) {
		fprintf(stderr, "Usage: %s <filter> <input directory> <output directory> <region file>  <number of input files> \n", argv[0]);
		fprintf(stderr, "Example: %s salmon /home/2004-30-13/run /home/2004-30-13/process habitat_regions3.dat 7\n", argv[0]);
		exit(1);
    }
    ...
    if(!strncmp(filter, "salmon", 20)){
		...
		fprintf(stderr, "Filter not recognized\n", fname);
		exit(1);
	}
...
/* Read region information */
    //sprintf(fname, "%s/%s", inputDIR, argv[4]);
    ...
    sprintf(fname, "%s/%s", outputDIR, "habitat_sal_MIN.61");
    if ((fhab_sal0 = fopen(fname, "w")) == NULL) {
		fprintf(stderr, "Unable to open file %s\n", fname);
		exit(1);
    }
    if (ElioPutHeader(fhab_sal0, &oh)) {
		fprintf(stderr, "Unable to write header to %s\n", fname);
		exit(1);
    }

    sprintf(fname, "%s/%s", outputDIR, "habitat_sal_MAX.61");
    if ((fhab_sal1 = fopen(fname, "w")) == NULL) {
		fprintf(stderr, "Unable to open file %s\n", fname);
		exit(1);
    }
    if (ElioPutHeader(fhab_sal1, &oh)) {
		fprintf(stderr, "Unable to write header to %s\n", fname);
		exit(1);
    }
    ...
    sprintf(fname, "%s/%s", outputDIR, "habitat_tmp_MIN.61");
    if ((fhab_tmp0 = fopen(fname, "w")) == NULL) {
		fprintf(stderr, "Unable to open file %s\n", fname);
		exit(1);
    }
    if (ElioPutHeader(fhab_tmp0, &oh)) {
		fprintf(stderr, "Unable to write header to %s\n", fname);
		exit(1);
    }

    sprintf(fname, "%s/%s", outputDIR, "habitat_tmp_MAX.61");
    if ((fhab_tmp1 = fopen(fname, "w")) == NULL) {
		fprintf(stderr, "Unable to open file %s\n", fname);
		exit(1);
    }
    if (ElioPutHeader(fhab_tmp1, &oh)) {
		fprintf(stderr, "Unable to write header to %s\n", fname);
		exit(1);
    }
    ...
    sprintf(fname, "%s/%s", outputDIR, "habitat_vel_MIN.61");
    if ((fhab_vel0 = fopen(fname, "w")) == NULL) {
		fprintf(stderr, "Unable to open file %s\n", fname);
		exit(1);
    }
    if (ElioPutHeader(fhab_vel0, &oh)) {
		fprintf(stderr, "Unable to write header to %s\n", fname);
		exit(1);
    }

    sprintf(fname, "%s/%s", outputDIR, "habitat_vel_MAX.61");
    if ((fhab_vel1 = fopen(fname, "w")) == NULL) {
		fprintf(stderr, "Unable to open file %s\n", fname);
		exit(1);
    }
    if (ElioPutHeader(fhab_vel1, &oh)) {
		fprintf(stderr, "Unable to write header to %s\n", fname);
		exit(1);
    }
...
/* Sum up */
	...
	/* Do the combined analysis (based on vertically averaged hab params)*/
				...
				//printf("A %d %d %.2f %.2f %.2lf %.2lf %.2lf %.2lf %.2lf %.2lf\n", j, k, elv.d[k], helv.d[k], min_sal, max_sal, avg_sal, min_vel, max_vel, avg_vel);
				//printf("B %d %d %.2lf %.2lf %.2lf %.2lf %.2lf %.2lf %.2lf\n", j, k, hab_elv[k], hab_sal0[k], hab_sal1[k], hab_sal2[k], hab_vel0[k], hab_vel1[k], hab_vel2[k]);
		     }
//printf("%d %d %f %lf %lf %lf\n", j, k, elv.d[k], avg_tmp, min_tmp, max_tmp);
    ...
    ElioPutTimeStep(fhab_sal0, &oh, out);

    for (k = 0; k < helv.np; k++) {
		out.d[k] = hab_sal1[k];
    }
    ElioPutTimeStep(fhab_sal1, &oh, out);
    ...
    ElioPutTimeStep(fhab_tmp0, &oh, out);
    for (k = 0; k < helv.np; k++) {
		out.d[k] = hab_tmp1[k];
    }
    ElioPutTimeStep(fhab_tmp1, &oh, out);
    for (k = 0; k < helv.np; k++) {
		out.d[k] = hab_tmp2[k];
    }
    ...
    ElioPutTimeStep(fhab_vel0, &oh, out);
    for (k = 0; k < helv.np; k++) {
		out.d[k] = hab_vel1[k];
    }
    ElioPutTimeStep(fhab_vel1, &oh, out);
    for (k = 0; k < helv.np; k++) {
		out.d[k] = hab_vel2[k];
    }
...
/* Write out summary quantities */
	...
	hab_reg[j].hab_sal0 = 0.0;
	hab_reg[j].hab_sal1 = 0.0;
	hab_reg[j].hab_sal2 = 0.0;
	hab_reg[j].hab_tmp0 = 0.0;
	hab_reg[j].hab_tmp1 = 0.0;
	hab_reg[j].hab_tmp2 = 0.0;
	hab_reg[j].hab_vel0 = 0.0;
	hab_reg[j].hab_vel1 = 0.0;
...
/* Sum up for each node in the element */
		...
		hab_reg[j].hab_sal0 += hab_sal0[n1] / helv.etype[element] * el_areas[element];
		hab_reg[j].hab_sal1 += hab_sal1[n1] / helv.etype[element] * el_areas[element];
		hab_reg[j].hab_sal2 += hab_sal2[n1] / helv.etype[element] * el_areas[element];
		hab_reg[j].hab_tmp0 += hab_tmp0[n1] / helv.etype[element] * el_areas[element];
		hab_reg[j].hab_tmp1 += hab_tmp1[n1] / helv.etype[element] * el_areas[element];
		hab_reg[j].hab_tmp2 += hab_tmp2[n1] / helv.etype[element] * el_areas[element];
		hab_reg[j].hab_vel0 += hab_vel0[n1] / helv.etype[element] * el_areas[element];
		hab_reg[j].hab_vel1 += hab_vel1[n1] / helv.etype[element] * el_areas[element];
...
//if (j == 0) printf("%d %d %d %.0lf %.0lf %.0lf %.0lf %.0lf %.0lf %.0lf %.0lf %.0lf\n", j, k, element, hab_reg[j].hab_elv, hab_reg[j].hab_sal0, hab_reg[j].hab_sal1, hab_reg[j].hab_sal2, hab_reg[j].hab_vel0, hab_reg[j].hab_vel1, hab_reg[j].hab_vel2, hab_reg[j].hab_all, hab_reg[j].area);
	}
...
/* Adjust by the area of the region */
	...
	hab_reg[j].hab_sal0 /= hab_reg[j].area;
	hab_reg[j].hab_sal1 /= hab_reg[j].area;
	hab_reg[j].hab_sal2 /= hab_reg[j].area;
	hab_reg[j].hab_tmp0 /= hab_reg[j].area;
	hab_reg[j].hab_tmp1 /= hab_reg[j].area;
	hab_reg[j].hab_tmp2 /= hab_reg[j].area;
	hab_reg[j].hab_vel0 /= hab_reg[j].area;
	hab_reg[j].hab_vel1 /= hab_reg[j].area;
    ...
    fprintf(freg_hab, "%d\n", nreg);
    fprintf(freg_hab,
	    "Region \t Habitat (Elevation criterion) \t Habitat (Minimum Salinity Criterion) \t Habitat (Maximum Salinity Criterion) \t Habitat (Average Salinity Criterion) \t Habitat (Minimum Velocity Criterion) \t Habitat (Maximum Velocity Criterion) \t Habitat (Average Velocity Criterion) \t Habitat (Minimum Temperature Criterion) \t Habitat (Maximum Temperature Criterion) \t Habitat (Average Temperature Criterion) \t Habitat (Average Combined Criterion) \n");
    for (j = 0; j < nreg; j++) {
	fprintf(freg_hab, "%s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", hab_reg[j].id, hab_reg[j].hab_elv, hab_reg[j].hab_sal0, hab_reg[j].hab_sal1, hab_reg[j].hab_sal2, hab_reg[j].hab_vel0, hab_reg[j].hab_vel1, hab_reg[j].hab_vel2, hab_reg[j].hab_tmp0, hab_reg[j].hab_tmp1, hab_reg[j].hab_tmp2, hab_reg[j].hab_all);
    }
