function [] = z0_comp_with_residual(firstPath, secondPath) 
...
%	set(ph(1), 'ZDataSource', 'z');
	...
	set(h(2), 'position', [sp_pos(1) sp_pos(2) sp_pos(3) sp_pos(4)]);
...
% Residual values
	h(3)  = subplot(1,3,3);
	sp_pos = get(h(3),'position');
	set(h(3), 'position', [sp_pos(1)+0.05 sp_pos(2) sp_pos(3) sp_pos(4)]);
	z3  = ssh_03.data(1,i).val-ssh_0.data(1,i).val; 
	ph(3) = trisurf(hgrid.elem(:,3:5), hgrid.x, hgrid.y, z3, 'EdgeColor', 'none');
	set(gca,'YAxisLocation', 'right');	
	set(gca,'fontsize',8);
%	set(ph(2), 'ZDataSource', 'z');
	view(0,90);
	...
	%load mycmap;		% Test of crappy looking colorbar without gradient
	%colormap(cm);
	hcb = colorbar('horiz');
