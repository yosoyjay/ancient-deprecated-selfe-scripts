...
if nSedClasses == 0
    ...
    for day=startDay:endDay
        ...
        for sed = 1:nSedClasses
            ...
            exp = sprintf('hSed%d=sz_readHeader(sedPath);', sed);
            eval(exp);
        end
        gr.vgrid = hSalt.vgrid;
        ...
        for i=1:nSteps
            ...
            [saltData varTs] = sz_readTimeStep(hSalt,i);
            for sed = 1:nSedClasses
                exp = sprintf('[sedData%d varTs] = sz_readTimeStep(hSed%d,%d);', sed, sed, i);
                eval(exp);
            end
            ...
            dSalt = map_sz2hts(hSalt, saltData);
            for sed = 1:nSedClasses
                exp = sprintf('dSed%d = map_sz2hts(hSed%d, sedData%d);', sed, sed, sed);
                eval(exp);
            end
            %dHvel = map_sz2hts(hHvel, hvelData(:,1));
            ...
            salt = ob.xy.H*double(dSalt);
            for sed = 1:nSedClasses
                exp = sprintf('sed%d = ob.xy.H*double(dSed%d);', sed, sed);
                eval(exp);
            end
