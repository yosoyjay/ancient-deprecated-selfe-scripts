function [k_flow, k_wind, vO2, eta] = air_water_diffusion_optimize(runPath, startDay, endDay, method)
% [k_flow, k_wind, vO2] = air_water_diffusion(runPath, startDay, endDay, method)
%
...
%   method - Method of calculating k_flow
%		1.
%		2.
%		3.
%
...
for day = startDay:endDay
	...
	%hSalt = sz_readHeader([runPath '/outputs/' num2str(day) '_salt.63']);
	...
	% k2 is per day and not temp corrected, it's at 20c
	% k2 = k2,20(1.025)^(T-20) T is celcius 
	% k2 = 3.93 * V^0.5 * D^(-1.5) @ 20c
	% k2*depth end up with meters per second
	% 0.4 m/h
	...
	k_wind(day, :, :) = reshape(0.31.*winds_mag.*winds_mag.*(sc./660).^(-0.5), ...
	                            size(winds_mag,1), size(winds_mag,3))';	
