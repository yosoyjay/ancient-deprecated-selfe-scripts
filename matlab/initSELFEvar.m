function theResult = initSELFEvar(varIn)
...
%open files
dat.param=read_param2ha(dat.param_file);
...
% create a time index
dat.tstartFiles=dat.iter.*(dat.filenums-1).*dat.nspool+1;
...
for i=1:length(dat.tstartFiles)
    ...
    dat.time(idx,1)= dat.tstartFiles(i)+(1:dat.iter)*dat.nspool;  %time in seconds of outputs, file index
