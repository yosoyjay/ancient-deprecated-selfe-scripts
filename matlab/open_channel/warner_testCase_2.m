...
%		colormap(bluewhitered);
    ...
    plot(x(:,1:3), sz(:,1:3), 'color', [0.7 0.7 0.7], 'linewidth', 0.1);
		...
		set(gca,'xticklabel',0:2.5:10);

		% Mixed layer depth 
%		colormap(jet);
		h(2) = subplot(6,1,2);
		sp_pos = get(h(2), 'position'); hold on;
		mixLen = zeros(size(kine));
		for xPos = 1:size(kine,1)
			depth = find(kine(xPos,:) < 10e-5, 1, 'last');
			mixLen(xPos,depth) = 1;
		end
		contour(x,sz,mixLen);
		colorbar;
    %plot(x, sz, 'color', [0.7 0.7 0.7], 'linewidth', 0.1);
    plot(x(:,end), sz(:,end), 'color', 'b', 'linewidth', 0.1);
