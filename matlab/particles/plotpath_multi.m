function h = plotpath_multi(format,aux,reltime,titlestr,clustsz,dims,fig,colr,obs,obs_times)
...
[pathdata.lon,pathdata.lat] = convm2ll(pathdata.x,pathdata.y);
...
if size(dims, 1) == 0
    ...
    xmin = min([min(pathdata.lon), dims(1,1)])-0.01;
    xmax = max([max(pathdata.lon), dims(2,1)])+0.01;
    ymin = min([min(pathdata.lat), dims(1,2)])-0.01;
    ymax = max([max(pathdata.lat), dims(2,2)])+0.01;
end
...
switch true
    case strcmp(format(1:4),'simu')|strcmp(format(1:4),'Simu')
        ...
        material dull
        h.bnd = plotbnd(fgll);
        h.path = plot(pathdata.lon',pathdata.lat','Color', colr);
        h.min15 = plot(pathdata.lon,pathdata.lat,'.', 'Color', colr);
        h.stpnt = plot(pathdata.lon(:,1),pathdata.lat(:,1),'ok');
        h.endpnt = plot(pathdata.lon(:,end),pathdata.lat(:,end),'.k');
        firsthour = 4 - floor(mod(rem(reltime/3600,1)*4+1,4));
        h.hourly = plot(pathdata.lon(:,firsthour:4:end),pathdata.lat(:,firsthour:4:end),'ok');
        ...
        if length(obs)>0
            ...
            for tt = 1:4:length(obs_times)
                dvec = datevec(obs_times(tt));
                text(obs(tt,1), obs(tt,2), datestr(obs_times(tt), 'mm-dd-yy HH:MM'));
            end
        end
        ...
        latdec = rem(pathdata.lat(moving(1)-1:end),1).*60;
        londeg = ceil(pathdata.lon(moving(1)-1:end));
        londec = rem(-pathdata.lon(moving(1)-1:end),1).*60;
    ...
    case strcmp(format(1:4),'tran')|strcmp(format(1:4),'Tran')
        ...
        plot(pathdata.lon(:,end),pathdata.lat(:,end),'.k');
        plot(pathdata.lon(:,end),pathdata.lat(:,end),'.k')
        plot(pathdata.lon(:,4:4:end),pathdata.lat(:,4:4:end),'ok')
        east = find(pathdata.lon(:,1)==max(pathdata.lon(:,1)));
    ...
    case strcmp(format(1:4),'comp')|strcmp(format(1:4),'Comp')
        ...
        for i = 1:drops
            ...
            for j = 1:reps
                ...
                box on
                if j==1
                    labstr = {[num2str(floor(relpos.lat(i))) ' ' num2str(rem(relpos.lat(i),1)*60,'%6.4f')];  ...
							  [num2str(ceil(relpos.lon(i))) ' ' num2str(-rem(relpos.lon(i),1)*60,'%6.4f')]};
                    ylabel(labstr)
                end
                if i==drops
