...
% roughly define a box - The same as above? Nope, slightly smaller than those used in the indices only for x
% These are x,y values of the grid
x_min=3.31e5;
x_max=3.51e5;
y_min=2.71e5;
y_max=3.09e5;
...
% isle 1
isle_8_x=[xDbnd(indx(308:347));xDbnd(indx(308))];
isle_8_y=[yDbnd(indx(308:347));yDbnd(indx(308))];

% isle 2
isle_9_x=[xDbnd(indx(348:end));xDbnd(indx(348))];
isle_9_y=[yDbnd(indx(348:end));yDbnd(indx(348))];

% estuary boundary
E_bnd_x=[xDbnd(indx(45:215)); xDbnd(indx(45))];
E_bnd_y=[yDbnd(indx(45:215)); yDbnd(indx(45))];

BB_bndx=[xDbnd(indx(45:270)); xDbnd(indx(45)); NaN; isle_8_x; NaN; isle_9_x]; 
BB_bndy=[yDbnd(indx(45:270)); yDbnd(indx(45)); NaN; isle_8_y; NaN; isle_9_y];
...
%for irs=hires_set,
  ...
  fprintf(fid,'%f %i %i %i %i %i\n',0.1,fix(irs)+31,150,72,576,100);
  %fprintf(fid,'%i \n',1);  % Not used, native to Scott's implementation
  fprintf(fid,'%i \n',tot_drft);
 ...
 % locations that are within the region.
	  ...
	  temp_inpoly=inpolygon(xrnds,yrnds,BB_bndx,BB_bndy);
	  ipt_inreg=find(temp_inpoly==1);
      %ipt_inreg=find(inpolygon(xrnds,yrnds,BB_bndx,BB_bndx)==1);
