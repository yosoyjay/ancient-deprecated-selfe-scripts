function h = plotpath(format,aux,reltime,titlestr,clustsz,dims,obs,obs_times)
...
if size(dims, 1) == 0
    ...
    xmin = min([min(pathdata.x), dims(1,1)])-0.01;
    xmax = max([max(pathdata.x), dims(2,1)])+0.01;
    ymin = min([min(pathdata.y), dims(1,2)])-0.01;
    ymax = max([max(pathdata.y), dims(2,2)])+0.01;
end
if (length(obs)>0)
    if (min(obs(:,1))<xmin)
        xmin = min(obs(:,1));
