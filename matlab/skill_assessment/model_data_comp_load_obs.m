function [model, obs, obsInt, sa] = model_data_comp(runDirs, startDay, endDay, file, stats, obsData)
% Script extracts model and loads observation data for the time given and returns the
...
%
% Input:
%   runDirs   - Cell array of run directories to compare multiple runs 
%            - '/path/to/run/runName/run' - runName will be used in struct
...
%	obsDta - Path to *.mat data with all stations in appropriate station
%
% Ouput:
...
%       - If there is more than 1 run directory in runDirs
...
if length(runDirs) == 1
    model.run = get_model_station_data(runDirs{1}, startDay, endDay, stats, file);
else
...
endTime = addtodate(startTime, endDay-startDay+1, 'day');
%endTime = addtodate(endTime, dt, 'second');
%datestr(startTime)
%datestr(endTime)

% Get observation data 
%[obs, obsInt] = get_obs_station_data(startTime, endTime, stats, file, dt);
obsInt = eval(sprintf('load %s', obsData));
